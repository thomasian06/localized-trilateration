...
[bias, r, rel_power] = ...
    get_noisy_measurements(t, xtrue, pseudolites, 1e-6, sig_r0, 0.00);
...
for i = 2:T
    ...
    % Dynamics update
    particles(:, 1:2) = particles(:, 1:2) + 30*randn(nparticles, 2);
    particles(:, 3) = particles(:, 3) + 1e-7*randn(nparticles, 1);
    
    ss = rel_power(:, i) * sig_r0;
    dists = sqrt(rel_power(:, i));
    ...
    w = w .* normpdf(particles(:, 3), mean([b1, b2, b3]),...
        mean(rel_power(:, i))*std([b1, b2, b3]));
    if sum(w) ~= 0
        w = w / sum(w);
    else
